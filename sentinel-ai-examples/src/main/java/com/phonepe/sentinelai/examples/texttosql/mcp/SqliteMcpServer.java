...
@Slf4j
@Command(
        name = "sqlite-mcp-server", mixinStandardHelpOptions = true, description = "MCP server exposing SQLite operations over stdio or SSE")
public class SqliteMcpServer implements Callable<Integer> {
    ...
    @Option(
            names = {
                    "--db-path"
            }, required = true, description = "Absolute path to the SQLite database file")
    private String dbPath;

    @Option(
            names = {
                    "--transport", "-T"
            }, description = "Transport mode for the MCP server: ${COMPLETION-CANDIDATES}. Default: STDIO", defaultValue = "STDIO")
    private TransportMode transport;

    @Option(
            names = {
                    "--port", "-p"
            }, description = "HTTP port to which the MCP server should bind when using --transport=SSE (default: "
                    + DEFAULT_SSE_PORT + ")", defaultValue = "" + DEFAULT_SSE_PORT)
    private int port;
