...
public class AgentSessionExtension<R, T, A extends Agent<R, T, A>> implements AgentExtension<R, T, A> {
    ...
    @Builder
    public AgentSessionExtension(ObjectMapper mapper, @NonNull SessionStore sessionStore,
            AgentSessionExtensionSetup setup, List<MessagePersistencePreFilter<R>> historyModifiers,
            List<MessageSelector> messageSelectors) {
        ...
        this.historyModifiers = new CopyOnWriteArrayList<>(Objects.requireNonNullElseGet(historyModifiers, () -> List
                .of(new SystemPromptRemovalPreFilter<>(), new FailedToolCallRemovalPreFilter<>())));
        this.messageSelectors = new CopyOnWriteArrayList<>(Objects.requireNonNullElseGet(messageSelectors, () -> List
                .of(new UnpairedToolCallsRemover())));
    }
    ...
    public List<FactList> facts(R request, AgentRunContext<R> context, A agent) {
        ...
        return sessionStore.session(sessionId)
                .map(sessionSummary -> List.of(new FactList("Information about session %s".formatted(sessionId), List
                        .of(new Fact("A summary of the conversation in this session", sessionSummary.getSummary())))))
                .orElse(List.of());
    }
    ...
    public List<AgentMessage> messages(AgentRunContext<R> context, A agent, R request) {
        ...
        log.debug("Reading messages for session {} since last summarized message id {}", sessionId,
                lastSummarizedMessageId);
        final var agentMessages = readMessagesSinceId(sessionStore, setup, sessionId, lastSummarizedMessageId, true,
     ...
     */
    protected UserPrompt buildSummarizationUserPrompt(final AgentRunContext<R> context, final String sessionId,
            final String currentSummary,
            final BiScrollable<AgentMessage> sessionMessages) throws JsonProcessingException {
        return new UserPrompt(sessionId, context.getRunId(), """
                Generate a %d character summary of the conversation between user and \
                agent from the following messages.
                - Summary till now: %s,
                - Messages JSON: %s""".formatted(setup.getMaxSummaryLength(), Objects.requireNonNullElse(currentSummary,
                "Does not exist as this is the first compaction"), mapper.writeValueAsString(sessionMessages)),
                LocalDateTime.now());
    }
    ...
    private boolean isCompationNeeded(final Agent.ProcessingCompletedData<R, T, A> data,
            final List<AgentMessage> messages, final AgentSetup agentSetup) {
        if (data.getOutput().getError().getErrorType().equals(ErrorType.LENGTH_EXCEEDED)) {
        ...
        final var contextWindowSize = agentSetup.getModelSettings().getModelAttributes().getContextWindowSize();
        ...
        log.debug(
                "Automatic summarization evaluation: estimatedTokenCount={}, contextWindowSize={}, " + "threshold={}%, currentBoundary={}, needsSummarization={}",
    ...
    private void saveMessages(AgentRunContext<R> context, List<AgentMessage> messages) {
        ...
        var newMessages = messages.stream().filter(message -> message.getRunId().equals(context.getRunId())).toList();
    ...
    private void saveSummary(final String sessionId, final AgentRunContext<R> context, final ExtractedSummary summary,
            final String newestMessageId, final String lastSummarizedMessageId) {
        try {
            ...
            if (lastSummarizedMessageId != null && !Objects.equals(lastSummarizedMessageId, existingSessionMessageId)) {
                log.warn(
                        "Skipping summary save as the newest message id {} does not match existing session's last summarized message id {} for session: {}",
                        newestMessageId, existingSessionMessageId, sessionId);
                return;
            }
            final var updated = sessionStore.saveSession(new SessionSummary(sessionId, summary.getTitle(), summary
                    .getSessionSummary(), summary.getKeywords(), newestMessageId, AgentUtils.epochMicro()))
                    .orElse(null);
            log.info("Session summary: {}", updated);
        }
        catch (Exception e) {
            log.error("Error converting json node to memory output. Error: %s Summary: %s".formatted(e.getMessage(),
                    summary), e);
        }
    }
    ...
    private void summarizeConversation(Agent.ProcessingCompletedData<R, T, A> data) {
        try {
            ...
            log.error("Summarization process was interrupted for session: {}", AgentUtils.sessionId(data.getContext()));
        }
        catch (ExecutionException e) {
            final var error = AgentUtils.rootCause(e);
            log.error("Execution error during conversation summarization for session {}: {}", AgentUtils.sessionId(data
                    .getContext()), error.getMessage(), error);
        }
    }

    private void summarizeConversationImpl(
            Agent.ProcessingCompletedData<R, T, A> data) throws JsonProcessingException, InterruptedException, ExecutionException {
        ...
        messages.add(new com.phonepe.sentinelai.core.agentmessages.requests.SystemPrompt(sessionId, context.getRunId(),
                mapper.writeValueAsString(buildSummarizationSystemPrompt(sessionId)), false, null));
        final var lastSummarizedMessageId = AgentUtils.getIfNotNull(existingSession,
                SessionSummary::getLastSummarizedMessageId, null);
        final var sessionMessages = readMessagesSinceId(sessionStore, setup, sessionId, lastSummarizedMessageId, false,
                messageSelectors);
        messages.add(buildSummarizationUserPrompt(context, sessionId, AgentUtils.getIfNotNull(existingSession,
                SessionSummary::getSummary, null), sessionMessages));
        ...
        final var summary = MessageCompactor.compactMessages(data.getAgent().name(), sessionId, AgentUtils.userId(
                context), agentSetup, mapper, context.getModelUsageStats(), messages).join().orElse(null);
        if (null == summary) {
            ...
            final var newestMessageId = existingSession == null ? null : sessionMessages.getItems()
                    .stream()
