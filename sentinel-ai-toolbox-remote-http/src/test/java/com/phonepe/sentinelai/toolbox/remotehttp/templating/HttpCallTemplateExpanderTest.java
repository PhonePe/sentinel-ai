...
class HttpCallTemplateExpanderTest {
    ...
    void test(WireMockRuntimeInfo wiremock) {
        ...
        final var toolSource = InMemoryHttpToolSource.builder()
                .mapper(mapper)
                .build()
                .register(upstream, TemplatizedHttpTool.builder()
                        .metadata(HttpToolMetadata.builder()
                                .name("getLocation")
                                .description("Get location of the user")
                                .parameters(Map.of("name", new HttpToolMetadata.HttpToolParameterMeta(
        ...
        final var response = tools.get(toolId).accept(new ExecutableToolVisitor<String>() {
            ...
            public String visit(ExternalTool externalTool) {
                return (String) externalTool.getCallable().apply(null, toolId, """
                        {
                            "name" : "santanu"
                        }
                        """).response();
    ...
    void testHandlebar(WireMockRuntimeInfo wiremock) {
        ...
        stubFor(post(urlEqualTo("/api/v1/location")).withRequestBody(containing("santanu")).willReturn(jsonResponse("""
                ...
                """, 200)));
        stubFor(post(urlEqualTo("/api/v1/device")).withRequestBody(containing("santanu")).willReturn(jsonResponse("""
                {
                "device" : "android"
                }
                """, 200)));
        stubFor(post(urlEqualTo("/api/v1/device")).withRequestBody(containing("santanu.sinha"))
        ...
        final var handlebarFileContent = Files.readString(Paths.get(getClass().getClassLoader()
                ...
                .register(upstream, TemplatizedHttpTool.builder()
                        ...
                        .responseTransformations(ResponseTransformerConfig.builder()
                                ...
                                .config("""
                                        ...
                                        """)
                                .build())
                        .build(), TemplatizedHttpTool.builder()
                                .metadata(HttpToolMetadata.builder()
                                        .name("getDevice")
        ...
        final var response1 = tools.get(locationToolId).accept(new ExecutableToolVisitor<String>() {
            @Override
            public String visit(ExternalTool externalTool) {
                return (String) externalTool.getCallable().apply(null, locationToolId, """
                        {
                            "name" : "santanu"
                        }
                        """).response();
        ...
        final var response2 = tools.get(deviceToolId).accept(new ExecutableToolVisitor<String>() {
            @Override
            public String visit(ExternalTool externalTool) {
                return (String) externalTool.getCallable().apply(null, deviceToolId, """
                        {
                            "name" : "santanu"
                        }
                        """).response();
        ...
        final var response3 = tools.get(device2ToolId).accept(new ExecutableToolVisitor<String>() {
            @Override
            public String visit(ExternalTool externalTool) {
                return (String) externalTool.getCallable().apply(null, device2ToolId, """
                        {
                            "name" : "santanu"
                        }
                        """).response();
