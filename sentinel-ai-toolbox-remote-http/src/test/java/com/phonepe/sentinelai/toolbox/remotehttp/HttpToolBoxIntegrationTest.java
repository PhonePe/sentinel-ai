...
class HttpToolBoxIntegrationTest {
    public static class SimpleAgent extends Agent<String, String, SimpleAgent> {
        ...
        public SimpleAgent(AgentSetup setup,
                           List<AgentExtension<String, String, SimpleAgent>> extensions,
                           Map<String, ExecutableTool> tools) {
            super(String.class,
                  "Greet the user and Respond to user queries.",
                  setup,
                  extensions,
                  tools);
        }
    ...
    void test(WireMockRuntimeInfo wiremock) {
        ...
        HttpToolReaders.loadToolsFromYAML(Path.of(Objects.requireNonNull(
                                                                         getClass()
                                                                                 .getResource("/templates.yml"))
                .toURI()), toolSource);
        final var toolBox = new HttpToolBox(upstream,
                                            okHttpClient,
                                            toolSource,
                                            objectMapper,
                                            TestUtils.getTestProperty(
