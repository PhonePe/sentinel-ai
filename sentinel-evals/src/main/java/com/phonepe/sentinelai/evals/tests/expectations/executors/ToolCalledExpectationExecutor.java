...
public class ToolCalledExpectationExecutor<R, T>
    ...
    public ToolCalledExpectationExecutor(ToolCalledExpectation<R, T> expectation, Agent<R, T, ?> agent) {
        super(expectation);
        this.agent = agent;
    }
    ...
    private static Optional<Map<String, Object>> parseArguments(String arguments) {
        try {
            ...
            return Optional.of(SerDe.mapper().readValue(arguments, MAP_TYPE));
        }
        catch (Exception e) {
            return Optional.empty();
        }
