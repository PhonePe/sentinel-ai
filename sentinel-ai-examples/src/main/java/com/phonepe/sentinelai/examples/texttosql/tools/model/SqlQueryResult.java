...
public record SqlQueryResult(
        @JsonPropertyDescription(
            "The SQL statement that was generated from the user provided natural-language request") String generatedSql,
        @JsonPropertyDescription(
            "Rows returned by the query. Each entry is a JSON string representing one row, "
                    ...
                    + "JSON type. Empty list if the query returned no rows or could not be executed.") List<String> results,
        @JsonPropertyDescription(
            "A human-readable summary of what was done, what the results mean,"
                    + " and any caveats or errors encountered.") String explanation,
        @JsonPropertyDescription(
            "Wall-clock time in milliseconds from query submission to result receipt.") long executionTimeMs
) {
}
