...
public class FullRunMessageSelector implements MessageSelector {
    private static final Set<AgentMessageType> TEXT_REQ_RES = Set.of(AgentMessageType.USER_PROMPT_REQUEST_MESSAGE,
            AgentMessageType.TEXT_RESPONSE_MESSAGE);
    private static final Set<AgentMessageType> SO_REQ_RES = Set.of(AgentMessageType.USER_PROMPT_REQUEST_MESSAGE,
    ...
    public List<AgentMessage> select(String sessionId, List<AgentMessage> messages) {
        ...
        final var runMessages = messages.stream()
                .collect(groupingBy(AgentMessage::getRunId, mapping(AgentMessage::getMessageType, toUnmodifiableSet())))
                .entrySet()
                .stream()
                .filter(e -> e.getValue().containsAll(TEXT_REQ_RES) || e.getValue().containsAll(SO_REQ_RES))
