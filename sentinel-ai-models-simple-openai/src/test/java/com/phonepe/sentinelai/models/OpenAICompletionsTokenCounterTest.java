...
class OpenAICompletionsTokenCounterTest {
    ...
    void testEstimateTokenCountAssistantTextResponse() {
        ...
        int expected = TokenCountingConfig.DEFAULT.getAssistantPrimingOverhead() + TokenCountingConfig.DEFAULT
                .getMessageOverHead() + countTokens("ASSISTANT") + countTokens(content);

        assertEquals(expected, tokenCounter.estimateTokenCount(List.of(assistantResponse), TokenCountingConfig.DEFAULT,
                EncodingType.CL100K_BASE));
    }

    @Test
    void testEstimateTokenCountEmptyMessages() {
        assertEquals(TokenCountingConfig.DEFAULT.getMessageOverHead(), tokenCounter.estimateTokenCount(List.of(),
                TokenCountingConfig.DEFAULT, EncodingType.CL100K_BASE));
    }
    ...
    void testEstimateTokenCountGenericText() {
        ...
        assertEquals(expected, tokenCounter.estimateTokenCount(List.of(genericText), TokenCountingConfig.DEFAULT,
                EncodingType.CL100K_BASE));
    }
    ...
    void testEstimateTokenCountStructuredOutput() {
        ...
        assertEquals(expected, tokenCounter.estimateTokenCount(List.of(structuredOutput), TokenCountingConfig.DEFAULT,
                EncodingType.CL100K_BASE));
    }
    ...
    void testEstimateTokenCountSystemPrompt() {
        ...
        int expected = TokenCountingConfig.DEFAULT.getAssistantPrimingOverhead() + TokenCountingConfig.DEFAULT
                .getMessageOverHead() + countTokens("SYSTEM") + countTokens(content);

        assertEquals(expected, tokenCounter.estimateTokenCount(List.of(systemPrompt), TokenCountingConfig.DEFAULT,
                EncodingType.CL100K_BASE));
    }
    ...
    void testEstimateTokenCountToolCall() {
        ...
        int expected = TokenCountingConfig.DEFAULT.getAssistantPrimingOverhead() + TokenCountingConfig.DEFAULT
                .getMessageOverHead() + countTokens("ASSISTANT") + countTokens(toolCallId) + countTokens(
                        toolName) + TokenCountingConfig.DEFAULT.getFormattingOverhead() + countTokens(
                                arguments) + countTokens("null"); // Content is null in ToolCall, Objects.toString(null) is "null"

        assertEquals(expected, tokenCounter.estimateTokenCount(List.of(toolCall), TokenCountingConfig.DEFAULT,
                EncodingType.CL100K_BASE));
    }
    ...
    void testEstimateTokenCountToolCallResponse() {
        ...
        int expected = TokenCountingConfig.DEFAULT.getAssistantPrimingOverhead() + TokenCountingConfig.DEFAULT
                .getMessageOverHead() + countTokens("TOOL") + TokenCountingConfig.DEFAULT
                        .getFormattingOverhead() + countTokens(toolCallId) + countTokens(response);

        assertEquals(expected, tokenCounter.estimateTokenCount(List.of(toolCallResponse), TokenCountingConfig.DEFAULT,
                EncodingType.CL100K_BASE));
    }
    ...
    void testEstimateTokenCountUserPrompt() {
        ...
        assertEquals(expected, tokenCounter.estimateTokenCount(List.of(userPrompt), TokenCountingConfig.DEFAULT,
                EncodingType.CL100K_BASE));
    }
